    ...
    <div className="bg-gray-800 p-4 rounded-lg border border-gray-700">
      ...
      {agentRole === AgentRole.WATCHER && (
        <p className="text-xs text-yellow-300/80 mb-2 p-2 bg-yellow-900/30 rounded-md border border-yellow-700/50">
          Note: The core generation logic uses standardized internal templates
          for scenario creation and quality evaluation to ensure reliability.
          This prompt is for informational purposes and to set the agent's
          model.
        </p>
      )}
      {agentRole === AgentRole.BUILDER && (
        <p className="text-xs text-blue-300/80 mb-2 p-2 bg-blue-900/30 rounded-md border border-blue-700/50">
          Note: The model for the Builder Agent can also be configured directly
          in the chat panel for convenience.
        </p>
      )}
      <textarea
...
const BuilderAgentChat: React.FC<{ availableModels: string[] }> = ({
  availableModels,
}) => {
  const { activeWorkspace, updateConfig, updateBuilderMessages, setLastError } =
    useAppContext();
    ...
    try {
      ...
      const toolsString =
        config.tools.length > 0
          ...
          : "No tools defined.";
      const ragActionsString =
        config.ragActions.length > 0
          ? JSON.stringify(config.ragActions, null, 2)
          : "No RAG actions defined.";
      ...
      const prompt = config[AgentRole.BUILDER].prompt
        ...
        .replace(
          "{REFERENCE_DATA_SNIPPET}",
          (config.referenceData || "").substring(0, 500)
        )
        .replace(
          "{IS_UNSAFE_EXECUTION_ENABLED}",
          String(config.isUnsafeCodeExecutionEnabled)
        )
        .replace("{CURRENT_TOOLS}", toolsString)
        .replace("{CURRENT_RAG_ACTIONS}", ragActionsString);

      const response = await aiProvider.generateJsonContent(
        config[AgentRole.BUILDER].model,
        prompt,
        SchemaType.BUILDER_AGENT
      );

      if (
        response &&
        typeof response.response === "string" &&
        typeof response.configChanges === "object" &&
        response.configChanges !== null
      ) {
        ...
        const errorDetail = `Builder agent returned an invalid response shape. Expected { response: string, configChanges: object }, but got: ${JSON.stringify(
          response
        )}`;
        ...
        const assistantMessage: Message = {
          role: "assistant",
          content:
            "I encountered an internal error with my response format. Please try rephrasing your request.",
          timestamp: new Date().toLocaleTimeString(),
        };
        updateBuilderMessages((prev) => [...prev, assistantMessage]);
      }
    ...
    >
      <div className="space-y-4">
        {items.map((item, index) => (
          <div
            key={item.id}
            className="bg-gray-900/50 p-4 rounded-lg border border-gray-600 space-y-3"
          >
        ...
        <div className="text-xs text-yellow-300/80 mt-2 p-2 bg-yellow-900/30 rounded-md border border-yellow-700/50">
          <strong>Security Warning:</strong> The code you write will be executed
          by the system. Only use code from trusted sources. A simple SDK is
          provided with a `log(message)` function.
        </div>
      </div>
    </CollapsibleSection>
  );
...
export const ConfigurationTab: React.FC<ConfigurationTabProps> = () => {
  ...
  useEffect(() => {
    ...
    config.aiProvider,
    config.localAIConfig.baseURL,
    config.localAIConfig.apiKey,
    setLastError,
  ]);
  ...
  const handleLocalConfigChange = (
    field: keyof ProjectConfig["localAIConfig"],
    value: string
  ) => {
    updateConfig((prev) => ({
      ...prev,
  ...
  const setRagActions = (updater: React.SetStateAction<RAGAction[]>) => {
    updateConfig((prev) => ({
      ...prev,
      ragActions:
        typeof updater === "function" ? updater(prev.ragActions) : updater,
    }));
  };
  ...
  return (
    // <div className="grid grid-cols-1 lg:grid-cols-2 overflow-hidden h-full p-4 gap-4 bg-gray-900">
      ...
      <div className="lg:col-span-1 overflow-y-auto pr-2 space-y-4">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                onChange={(e) =>
                  handleConfigChange("referenceData", e.target.value)
                }
                rows={6}
          ...
          <div className="space-y-4">
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  handleConfigChange("aiProvider", e.target.value)
                }
                className="w-full bg-gray-700 border border-gray-600 rounded-md px-3 py-2 text-gray-200 focus:ring-blue-500 focus:border-blue-500"
              >
              ...
              <div className="border-t border-gray-600 pt-4 space-y-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleLocalConfigChange("baseURL", e.target.value)
                    }
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleLocalConfigChange("apiKey", e.target.value)
                    }
            ...
            {config.aiProvider === AIProviderType.GOOGLE_GEMINI && (
              <div className="text-xs text-blue-300/80 mt-2 p-2 bg-blue-900/30 rounded-md border border-blue-700/50">
                Note: The Google Gemini provider requires the `API_KEY`
                environment variable to be set in your frame's secrets.
              </div>
            )}
          </div>
        </CollapsibleSection>
        ...
        <RAGToolConfig
          ...
          newItem={() => ({
            ...
            name: "new_rag_action",
            description:
              "A new RAG action to retrieve information from a source.",
            code: DEFAULT_TOOL_CODE,
          })}
        />
